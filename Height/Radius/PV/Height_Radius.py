...
pvVer = getvar(ncfile, "pvo", timeidx=0)[0:nlev, ymin-expansion:ymax+1+expansion, \
...
wavenum = 6
pvVerWavP = np.zeros((wavenum, nlev, numr, numth))
for ilev in range(0, nlev):
    pvVerP = CarToPolar(to_np(pvVer[ilev, :, :]), rMin, rMax, numr, numth)
    pvVerWavP[:, ilev, :, :] = WaveDomain(pvVerP, wavenum, numr, numth) # do the fourier transform
########################################################################
...
for iwave in range(0, wavenum):
	...
	ac1 = ax1.contourf(dis_r, heightP[:, :, itheta], pvVerWavP[iwave, :, :, itheta], \
	...
	# the 0° is in East
	# loc = matplotlib.ticker.MaxLocator(21)
	# lvls = loc.tick_values(pvVerWavP[iwave, :, :, itheta].min(), pvVerWavP[iwave, :, :,  itheta].max())
	# ac2 = ax1.contour(dis_r, heightP[:, :, itheta], wVerWavP[iwave, :, :,  itheta], levels=lvls, 
	# 					cmap=cmaps.WhiteBlueGreenYellowRed, linestyles=np.where(lvls >= 0, "-", "--"),
	# 					linewidths=1)
	...
	cb.set_label(label='Potential Vorticity  PVU')
	...
	fig3.savefig('/gpfs3/whuang/Height/Radius/PV/Radius' + Time + '_wave'+str(iwave)+'_PV_in_'+str(itheta)+'.png', \
