...
wavenum = 6
# calculate the polar wind 
uthetaVerWavP = np.zeros((wavenum, nlev, numr, numth))
for ilev in range(0, nlev):
	uthetaVer, __ = polar_wind(uVer[ilev, :, :], vVer[ilev, :, :])
	uthetaVerP = CarToPolar(to_np(uthetaVer), rMin, rMax, numr, numth)
	uthetaVerWavP[:, ilev, :, :] = WaveDomain(uthetaVerP, wavenum, numr, numth) # do the fourier transform
...
for iwave in range(0, wavenum):
	...
	ac1 = ax1.contourf(dis_r, heightP[:, :, itheta], uthetaVerWavP[iwave, :, :, itheta], \
						cmap=cmaps.WhiteBlueGreenYellowRed, levels=np.arange(-10, 10.01, 1), \
						extend='both')
    ...
    # ac2 = ax1.contour(dis_r, heightP[:, :, itheta], wVerWavP[iwave, :, :,  itheta], levels=20, 
    #                   colors=np.where(lvls>=0, "r", "k"), linestyles=np.where(lvls >= 0, "-", "--"),
    #                   linewidths=0.5)
	...
	cb.set_label(label='Tangential Wind')
	...
	fig3.savefig('/gpfs3/whuang/Height/Radius/utheta/Radius' + Time + '_wave'+str(iwave)+'_utheta_in_'+str(itheta)+'.png', \
