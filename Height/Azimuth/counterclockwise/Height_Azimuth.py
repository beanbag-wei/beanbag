...
# GET THE AZIMUTHAL WIND AND RATIAL WIND IN POLAR GRID
...
minlatData = to_np(XLAT[yminData:ymaxData+1, xminData:xmaxData+1]).flatten().min()
maxlatData = to_np(XLAT[yminData:ymaxData+1, xminData:xmaxData+1]).flatten().max()
...
########################################################################
# Draw
# draw the wave domain of relative vorticity and circle on it
# according to the circle, draw the Height-Azimuth and Height-Radius
########################################################################
# draw the circle to find the appropriate radius in vertical-azimuthal
proj = ccrs.PlateCarree()
fig1 = plt.figure(figsize=(6, 6), dpi=600)

########################################################################
# Draw
# according to the red circle, draw the vertical-azimuth plane for W and RE sequence is E-S-W-N
# draw the height-azimuth of relative vorticity and w
########################################################################
# use ir and ilev to control the radius and level need to draw
# ir is the number of grid points from the center
# ra is the radius
ir = 100 
distance = np.multiply(360/numth, np.arange(0, numth, 1))
dis = np.repeat(distance, nlev, axis=0).reshape(numth, nlev).T
for iwave in range(0, wavenum):
	fig2 = plt.figure(figsize=(6, 6), dpi=600)
	ax1 = plt.axes([0.1, 0.1, 0.4, 0.6])

	# begin to draw
	ac1 = ax1.contourf(dis, heightP[:, ir, :], revorVerWavP[iwave, :, ir, :], \
						cmap=BlueOrange10_3.mpl_colormap, levels=18)
	...
	lvls = loc.tick_values(wVerWavP[iwave, :, ir, :].min(), wVerWavP[iwave, :, ir,  :].max())
	ac2 = ax1.contour(dis, heightP[:, ir, :], wVerWavP[iwave, :, ir,  :], levels=lvls, 
					cmap=RdGy_11_r.mpl_colormap, linestyles=np.where(lvls >= 0, "-", "--"),
					linewidths=0.75)

	# set the ylim, because the height of model varies with latitude and longitude. 3500m
	ax1.set_ylim(top=3500)

	# set the xy label
	ax1.set_xlabel('Azimuth (0$^{\circ}$ is East) counterclockwise', fontsize=8, labelpad=1)
	...
	fig2.savefig('/gpfs3/whuang/Height/Azimuth/Azimuth' + Time + '_wave'+str(iwave)+'_RV+w_in_'+str(ir)+'.png', pad_inches = 0, bbox_inches = 'tight')
